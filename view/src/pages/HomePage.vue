<template>
	<main class="pt-16 lg:pt-20 bg-indigo-500">
		...
		<div class="w-full p-10 flex justify-between items-center">
			...
			<img src="../assets/images/banner.gif" alt="banner" class="banner" />
		</div>

		<!-- Phần dịch vụ tra cứu cho khách hàng  -->
		<div
			class="py-14 flex flex-col gap-20 justify-center items-center w-full"
			id="search-service"
		>
					...
					<div class="bg-indigo-400 p-4" v-if="isSearchOrder">
						...
						>
							...
							<span class="flex flex-col">
								<h1 class="font-bold">Theo dõi quá trình đơn hàng:</h1>
								<div>
									<span class="flex gap-1">
										<h2 class="italic text-red-500">12/12/2023:</h2>
										<p>Đơn hàng đã được gửi tới điểm giao dịch.</p>
									</span>
									<span class="flex gap-1">
										<h2 class="italic text-red-500">12/12/2023:</h2>
										<p>Đơn hàng đã được gửi tới điểm giao dịch.</p>
		...
		>
			...
			<div class="grid grid-cols-2 gap-20 items-center justify-center px-48">
				<a
				...
				>
					...
					<p class="font-normal text-gray-700">
						Dịch vụ nhận gửi, vận chuyển và phát các loại thư, tài liệu, thư từ
						trong nước theo chỉ tiêu thời gian tiêu chuẩn, Không áp dụng với các
				...
				>
					...
					<p class="font-normal text-gray-700">
						Dịch vụ nhận gửi, vận chuyển và phát nhanh chứng từ hàng hóa, vật
						phẩm có độ ưu tiên cao nhất với chỉ tiêu thời gian toàn trình không
						quá 24 giờ [...]
					</p>
				</a>
				<a
				...
				>
					...
					<p class="font-normal text-gray-700">
						Dịch vụ nhận gửi, vận chuyển và phát các loại hàng hóa, bưu kiện
						không giới hạn mức trọng lượng, theo chỉ tiêu thời gian tiêu chuẩn,
						giá cước hợp lý [...]
					</p>
				</a>
				...
				>
					...
					<p class="font-normal text-gray-700">
						Dịch vụ nhận gửi, vận chuyển và phát các loại hàng hóa đặc biệt, quá
						trình tiếp nhận, đóng gói, quy định về tem nhãn cảnh báo riêng cho
						từng loại [...]
					</p>
				</a>
			</div>
		...
		<div class="py-10 flex flex-col gap-20 justify-center items-center w-full">
			...
			<div class="grid grid-cols-3 w-full">
				<div
					class="text-center bg-indigo-400 w-full text-white p-10 text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold"
				>
...
export default {
	...
	methods: {
		...
		async setTransactionProvinces() {
			await this.$store.dispatch("address/getProvinces");
			this.transactionProvinces = this.$store.getters["address/getProvinces"];
		},
	...
	computed: {
		...
		validateEstimateForm() {
			...
			if (this.sentProvince.value === "Tỉnh/Thành phố gửi") {
				...
				this.sentProvince.errorMessage = "Vui lòng chọn tỉnh/thành phố gửi";
				isValid = false;
			} else {
				this.sentProvince.isValid = true;
				this.sentProvince.errorMessage = "";
			...
			if (this.receiveProvince.value === "Tỉnh/Thành phố nhận") {
				...
				this.receiveProvince.errorMessage = "Vui lòng chọn tỉnh/thành phố nhận";
				isValid = false;
			} else {
				this.receiveProvince.isValid = true;
				this.receiveProvince.errorMessage = "";
			...
			if (this.receiveDistrict.value === "Quận/Huyện nhận") {
				...
				this.receiveDistrict.errorMessage = "Vui lòng chọn quận/huyện nhận";
				isValid = false;
			} else {
				this.receiveDistrict.isValid = true;
				this.receiveDistrict.errorMessage = "";
			...
			if (this.packageCategory.value === "Loại hàng") {
				...
				this.packageCategory.errorMessage = "Vui lòng chọn loại hàng hóa";
				isValid = false;
			} else {
				this.packageCategory.isValid = true;
				this.packageCategory.errorMessage = "";
			...
			if (this.packageWeight.value === null) {
				...
				this.packageWeight.errorMessage = "Vui lòng nhập khối lượng hàng hóa";
				isValid = false;
			} else {
				this.packageWeight.isValid = true;
				this.packageWeight.errorMessage = "";
		...
		validateTransactionForm() {
			...
			if (this.transactionProvince.value === "Tỉnh/Thành phố") {
				...
				this.transactionProvince.errorMessage = "Vui lòng chọn tỉnh/thành phố";
				isValid = false;
			} else {
				this.transactionProvince.isValid = true;
				this.transactionProvince.errorMessage = "";
